...
> **Purpose:** Copy-paste this entire document as a prompt to an AI coding agent (Cursor Agent, ChatGPT, Claude, etc.) to produce a self-contained go/no-go feasibility test for the research project.
>
> **What it produces:** A single Python script (or Jupyter notebook) that simulates a rotor under 4 conditions, computes bispectrum for each, and visually answers: *"Can HOS distinguish shaft cracks from misalignment in numerical simulations?"*
...
Build a Laval/Jeffcott-like rotor model:
