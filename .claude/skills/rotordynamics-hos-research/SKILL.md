...
3. The thesis-specific framing constraints (target journals, Sinha 2007 as validation reference, RK4 experimental extension).
...
- **`99_references/`** — the literature folder. Currently contains:
  - *Sinha (2007), "Higher Order Spectra for Crack and Misalignment Identification in the Shaft of a Rotating Machine"*, Structural Health Monitoring 6(4), 325–334. The paper that the thesis numerically validates against.
...
- **`01_article/01_publication_roadmap.md` §4.2** — names the eight literature categories the publishable paper needs (rotordynamics fundamentals, crack breathing, misalignment, HOS theory, HOS in rotating machinery, comparator methods, numerical fault diagnosis, ROSS / open-source rotordynamics) with target counts.
- **`01_article/02_executive_summary.md`** — short framing of the research gap; useful for Introduction-section calibration.
- **`01_article/04_rk4_working_plan.md`** — the experimental-validation extension on the Bently Nevada RK4 rotor kit.
- **`02_simula/05_synthesis.ipynb`** — the audit of the simulation pipeline. Cite for any methodological claim about the current state of the code.
...
**ROSS library**
- *Timbó et al.* — original ROSS paper. Cite as the open-source FEM rotordynamics tool used in the thesis.
...
The publishability calibration is set by `01_article/01_publication_roadmap.md §6.1`. From most-to-least preferred:
...
When drafting Methods, Introduction, or Discussion text, observe the constraints already locked into the planning documents:

- The thesis is a **numerical investigation** ("Numerical and Experimental Study" if the RK4 extension lands; see `01_article/04_rk4_working_plan.md`). Do not phrase the contribution as a theoretical advance.
- The validation reference is **Sinha (2007)**. The thesis aims to *reproduce* its experimental HOS features (Figs. 2, 3, 5, 7 for crack; 4, 6, 8 for misalignment) and *extend* the simulation work that Sinha §5 acknowledged could not be done for misalignment.
...
5. **Cross-link with `01_publication_roadmap.md §4.2`** — every new reference should fit into one of the eight category buckets (rotordynamics fundamentals, crack breathing, misalignment, HOS theory, HOS in rotating machinery, comparator methods, numerical fault diagnosis, ROSS).
...
- A new BibTeX entry for `00_proposta_de_trabalho/references.bib`.
